...
**两仓的这个文件逐字节相同**（SHA256
`712AEA6995962ED70801EF9B39FCA3CF5749B026DB7E0EFF392AA1310A45D8EE`，17939 字节）——
与 `check_figures.mjs` / `check_legend_convention.mjs` 同样的约定：
改一侧必须同步另一侧并比对哈希。

> **2026-09-26 更新（E-70）：该文件已判「四件事」，不是三件。** 上面三条之外，
> 新增第四条判据「**循环体里建了图，但同一个循环体内没有保存**」（`save_fig`
> 缩进错一格会跑到 `for` 外面 —— 循环建 N 张、只保存最后一张，而
> `check_figures.mjs` / `check_fig_names.mjs` / 验收层 / `py_compile`
> 四套门禁**结构上都看不见**这一类）。
>
> 上一条哈希 `90D2ECEF…` / 12530 字节是**三条判据时代**的。**哈希与字节数
> 会随判据增加而过时 —— 引用前重新算一遍，不要从文档抄**（同族：AGENTS 规则
> 里那些「文档里写死的数值一定会过时」）。
